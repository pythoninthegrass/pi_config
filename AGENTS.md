...
- Active default is set per machine via `PI_DEFAULT_PROVIDER` / `PI_DEFAULT_MODEL` in `.env`, rendered into `settings.json` at shell startup. Current defaults: `omlx` + `Qwen3.6-35B-A3B-bf16` (Studio, 128GB) / `Qwen3.6-35B-A3B-MLX-8bit` (MBP, 64GB), or `aperture` + `qwen3.8-flash-next:builder` (`mf`'s vLLM/llama-swap, routed through the tailnet `ai`/Aperture gateway at `http://ai/v1` for centralized auth, telemetry, and session tracking — see `~/git/tailscale_config/aperture.hujson`'s `mf` provider block for the upstream registration; the `:builder` suffix is required — llama-swap's `setParamsByID` in `docker/llama-swap/config.yaml` only injects `enable_thinking: false` for the exact suffixed ID, and the bare ID defaults to thinking-on per the model's HF card). All model IDs are listed in `models.json` / `models.yml.tpl`.
...
- **Planner/builder toggle (`tab`)**: `extensions/plan-mode/index.ts` swaps model, thinking level, and active tools together — planner is `github-copilot/claude-opus-4.8` (`thinking: high`, read-only tools), builder is `aperture/qwen3.8-flash-next:builder` (`thinking: off`, full tools). Same handler as `/plan` and `Ctrl+Alt+P`. Previously bound to `shift+tab`, but that key is reserved by pi core for `app.thinking.cycle` (default keybinding, see `earendil-works/pi`'s `packages/coding-agent/src/core/keybindings.ts`) — the extension's identical binding collided with it, so the toggle silently never fired. Moved to plain `tab` (mirrors opencode's native agent-cycle key), which extensions intercept before the editor's own tab-autocomplete handling; this means `tab` no longer autocompletes slash-commands/file-paths while the plan-mode extension is loaded. `keybindings.json`'s `ctrl+shift+t` remap for `app.thinking.cycle` is no longer required to free `shift+tab` but is left in place. Rebinding to `tab` still collided with the separate built-in `tui.input.tab` action (autocomplete), which pi's startup diagnostics flagged as an "Extension shortcut conflict" — functionally harmless (`CustomEditor.handleInput()` checks extension shortcuts before any built-in action, so the extension always won), but to silence the warning at the source `keybindings.json` explicitly unbinds it: `"tui.input.tab": []`, the same empty-array convention pi uses internally for actions with no default key. The planner model requires a GitHub Copilot OAuth login (`/login` inside pi). `github-copilot/gpt-6-astra` and `github-copilot/claude-opus-5` were tried first but both 400 with `model_not_supported` — real models per models.dev, but not yet in this account's Copilot entitlement (`~/.pi/agent/auth.json`'s `availableModelIds`) or in pi's/opencode's bundled catalogs; `claude-opus-4.8` is entitled today and works end-to-end. The commented-out alternatives stay in `extensions/plan-mode/index.ts` for whenever the account gets `gpt-6-astra`/`claude-opus-5` access.
