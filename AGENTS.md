...
- Active default is set per machine via `PI_DEFAULT_PROVIDER` / `PI_DEFAULT_MODEL` in `.env`, rendered into `settings.json` at shell startup. Current defaults: `omlx` + `Qwen3.6-35B-A3B-bf16` (Studio, 128GB) / `Qwen3.6-35B-A3B-MLX-8bit` (MBP, 64GB), or `aperture` + `qwen3.8-flash-next` (`mf`'s vLLM/llama-swap, routed through the tailnet `ai`/Aperture gateway at `http://ai/v1` for centralized auth, telemetry, and session tracking — see `~/git/tailscale_config/aperture.hujson`'s `mf` provider block for the upstream registration). All model IDs are listed in `models.json` / `models.yml.tpl`.
