...
- **Planner/builder toggle (`shift+tab`)**: `extensions/plan-mode/index.ts` swaps model, thinking level, and active tools together — planner is `github-copilot/gpt-6-astra` (`thinking: high`, read-only tools), builder is `aperture/qwen3.8-27b-fp8` (`thinking: off`, full tools). Same handler as `/plan` and `Ctrl+Alt+P`; `tab` still autocompletes, thinking-level cycling moved to `ctrl+shift+t` (`keybindings.json`) to free `shift+tab`. The planner model requires a GitHub Copilot OAuth login (`/login` inside pi) and, for a brand-new model like `gpt-6-astra`, may need one-time enablement in the VS Code Copilot Chat model picker before it stops returning `model_not_supported`.
