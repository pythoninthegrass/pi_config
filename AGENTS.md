...
- Active default is set per machine via `PI_DEFAULT_PROVIDER` / `PI_DEFAULT_MODEL` in `.env`, rendered into `settings.json` at shell startup. Current defaults: `omlx` + `Qwen3.6-35B-A3B-bf16` (Studio, 128GB) / `Qwen3.6-35B-A3B-MLX-8bit` (MBP, 64GB), or `aperture` + `qwen3.8-flash-next` (`mf`'s own llama-swap, hit directly over its Tailscale FQDN at `http://mf.sungrazer-vernier.ts.net:61515/v1` — no longer routed through the `ai`/Aperture gateway). All model IDs are listed in `models.json` / `models.yml.tpl`.
